...
import { Bell, Clock3, Settings, Share2, Smartphone, Timer, Utensils, PenLine } from "lucide-react";
...
const behaviors = [
  ...
  },
  {
    ...
    actionText: "더 안 먹어도 충분해요",
    responsePool: [
      "오, 생각했네요",
      "충분한지 한번 봤네요",
      "그 한 번이면 됐어요",
    ],
    displayText: "충분한데 손이 갈 때",
  },
  {
    key: "other",
...
const diagnosisCopy = {
  smartphone: {
    question1: "손에서 놓아도 되는데 왜 계속 보고 있을까요?",
    options1: [
      { label: "아직 뭔가 놓칠까 봐서", value: "bored" },
      { label: "잠깐만 보려다 이어져서", value: "just_try" },
      { label: "손이 익숙하게 가서", value: "habit" },
      { label: "머리를 쉬게 하고 싶어서", value: "stress" },
    ],
    question2: "보다 보면 길어지는 이유는 뭘까요?",
    options2: [
      { label: "잠깐만 보려 했어요", value: "just_try" },
      { label: "오늘은 좀 봐도 될 것 같아요", value: "today_ok" },
      { label: "시작하면 멈추기 싫어요", value: "cant_stop" },
      { label: "잘 모르겠어요", value: "unknown" },
    ],
  },

  delay: {
    question1: "해야 하는 걸 아는데 왜 아직 미루고 있을까요?",
    options1: [
      { label: "시작하기가 부담돼서", value: "bored" },
      { label: "어디서부터 할지 막막해서", value: "just_try" },
      { label: "늘 하던 흐름이라서", value: "habit" },
      { label: "피곤하거나 지쳐서", value: "stress" },
    ],
    question2: "미루기가 계속되는 이유는 뭘까요?",
    options2: [
      { label: "조금만 쉬려 했어요", value: "just_try" },
      { label: "오늘은 괜찮을 것 같아요", value: "today_ok" },
      { label: "미루다 보면 더 하기 싫어져요", value: "cant_stop" },
      { label: "잘 모르겠어요", value: "unknown" },
    ],
  },

  overeating: {
    question1: "충분히 먹었는데도 왜 더 먹고 있을까요?",
    options1: [
      { label: "아직 뭔가 아쉬워서", value: "bored" },
      { label: "맛있으니까 조금만 더", value: "just_try" },
      { label: "손이 익숙하게 가서", value: "habit" },
      { label: "마음이 허전해서", value: "stress" },
    ],
    question2: "계속 먹게 되는 이유는 뭘까요?",
    options2: [
      { label: "한 입만 더 하려 했어요", value: "just_try" },
      { label: "오늘은 괜찮을 것 같아요", value: "today_ok" },
      { label: "시작하면 멈추기 어려워요", value: "cant_stop" },
      { label: "잘 모르겠어요", value: "unknown" },
    ],
  },

  other: {
    question1: "그만해도 되는데 왜 계속하고 있을까요?",
    options1: [
      { label: "지금 멈추기 아쉬워서", value: "bored" },
      { label: "잠깐만 하려다 이어져서", value: "just_try" },
      { label: "그냥 하던 흐름이라서", value: "habit" },
      { label: "다른 생각을 피하고 싶어서", value: "stress" },
    ],
    question2: "계속하게 되는 이유는 뭘까요?",
    options2: [
      { label: "잠깐만 하려 했어요", value: "just_try" },
      { label: "오늘은 괜찮을 것 같아요", value: "today_ok" },
      { label: "시작하면 멈추기 싫어요", value: "cant_stop" },
      { label: "잘 모르겠어요", value: "unknown" },
    ],
...
export default function Page() {
  ...
  const [changeBehaviorConfirmOpen, setChangeBehaviorConfirmOpen] = useState(false);
...
useEffect(() => {
  const run = async () => {
    ...
    if (data.behavior_type === "other" && data.custom_behavior?.trim()) {
      setInterventionLine(`지금 ${data.custom_behavior.trim()} 생각이 나나요?`);
...
useEffect(() => {
  try {
    ...
    if (savedSettings && !isAuto) {
      ...
      setDiagnosis1(parsed.diagnosis1 ?? "");
      setDiagnosis2(parsed.diagnosis2 ?? "");
      if (
...
if (action === "stop") {
  ...
  const shareTitle = `나는 ${resultType.title}이래 😅`;

  const shareText =
    `${shareDescription}\n\n잠깐 생각하게 만드는 Loop Breaker`;

  const fullText =
    `나는 ${resultType.title}이래 😅\n\n` +
    `${shareDescription}\n\n` +
    `잠깐 생각하게 만드는 Loop Breaker\n\n` +
    `${url}`;
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white">
...
{step === "diagnosis" && (
  <Screen key="diagnosis">
    <div className="space-y-5 py-3">
      ...
      <div className="space-y-3">
        {currentDiagnosisCopy.options1.map((item) => (
          <button
            key={item.value}
            onClick={() => {
...
{step === "diagnosis2" && (
  <Screen key="diagnosis2">
    <div className="space-y-5 py-3">
      ...
      <div className="space-y-3">
        {currentDiagnosisCopy.options2.map((item) => (
          <button
            key={item.value}
            onClick={() => {
  ...
  <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 shadow-sm">
    <div className="space-y-2">
      ...
      <p className="text-base text-slate-700 leading-relaxed">
        행동을 바꾸면 생각한 횟수와 함께한 날짜가 새로 시작돼요.
      </p>
    </div>
    ...
    <div className="mt-4 grid grid-cols-2 gap-3">
      <button
        className="h-11 w-full rounded-2xl bg-slate-200 text-slate-900 hover:bg-slate-300"
        onClick={() => setChangeBehaviorConfirmOpen(false)}
      >
        취소
      </button>
      ...
      <button
        ...
        onClick={async () => {
          if (userId) {
            await supabase.from("pause_logs").delete().eq("user_id", userId);
          ...
          setEvents([]);
          setTotalStopCount(0);
          setWeeklyStopCount(0);
          setJoinedAt(new Date().toISOString());
          setDiagnosis1("");
          setDiagnosis2("");
          setChangeBehaviorConfirmOpen(false);
          setStep("behavior");
        }}
      >
        바꿀게요
